...
function xmldb_block_ai_chat_upgrade($oldversion) {
    ...
    if ($oldversion < 2025121300) {
        ...
        $table->add_field('timecreated', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('aicontext', XMLDB_KEY_FOREIGN, ['aicontext'], 'block_ai_chat_aicontext', ['id']);
        $table->add_index('pagetype-aicontext', XMLDB_INDEX_UNIQUE, ['pagetype', 'aicontext']);
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }
